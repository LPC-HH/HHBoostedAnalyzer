...
//tree->Draw("nBTaggedJets","fatJet1Pt>250 && fatJet2Pt>250 && fatJet1MassSD>50 && fatJet2MassSD>50 && fatJet1PNetXbb > 0.95 && fatJet2PNetXbb > 0.95")
...
void RunSelectHHTo4B(  std::vector<std::pair<std::vector<std::string>,std::string > > datafiles, std::vector<std::vector<std::pair<std::vector<std::string>,std::string > > > bkgfiles, std::vector<std::string> bkgLabels, std::vector<int> bkgColors, std::vector<float> bkgScaleFactors, int channelOption = 1, double totalLumi = 0, std::string label = "") {
  ...
  std::vector<TH1D*> histFoundAK4DeltaR;
  std::vector<TH1D*> histNotFoundAK4DeltaR;


  assert (inputfiles.size() == processLabels.size());
  for (uint i=0; i < inputfiles.size(); ++i) {
    histFoundAK4DeltaR.push_back(new TH1D(Form("histFoundAK4DeltaR_%s",processLabels[i].c_str()), "; #Delta R ; Number of Events", 50, 0, 1.5));
    histNotFoundAK4DeltaR.push_back(new TH1D(Form("histNotFoundAK4DeltaR_%s",processLabels[i].c_str()), "; #Delta R ; Number of Events", 50, 0, 1.5));
  ...
  for (uint i=0; i < inputfiles.size(); ++i) {
	...
	tree->SetBranchAddress("jet3Phi", &jet3Phi);
	tree->SetBranchAddress("jet4Pt", &jet4Pt);
	tree->SetBranchAddress("jet4Eta", &jet4Eta);
	tree->SetBranchAddress("jet4Phi", &jet4Phi);
	...
	for(UInt_t ientry=0; ientry < tree->GetEntries(); ientry++) {       	
	  ...
	  //Find the AK4 jets closest to fatJet1
	  float deltaR_ClosestToFatJet1 = -999;
	  float tmpDR_ClosestToFatJet1 = 9999;
	  float tmpDR_SecondClosestToFatJet1 = 9999;
	  float jet1Eta_ClosestToFatJet1 = 0;
	  float jet1Phi_ClosestToFatJet1 = 0;
	  float jet2Eta_ClosestToFatJet1 = 0;
	  float jet2Phi_ClosestToFatJet1 = 0;

	  double jet1ToFatJet1_dphi = (fabs(jet1Phi-fatJet1Phi) > TMath::Pi()) ? fabs(fabs(jet1Phi-fatJet1Phi) - 2*TMath::Pi()) : fabs(jet1Phi-fatJet1Phi);
	  double jet1ToFatJet1_dR = sqrt(pow(jet1Eta-fatJet1Eta,2)+pow(jet1ToFatJet1_dphi,2));
	  double jet2ToFatJet1_dphi = (fabs(jet2Phi-fatJet1Phi) > TMath::Pi()) ? fabs(fabs(jet2Phi-fatJet1Phi) - 2*TMath::Pi()) : fabs(jet2Phi-fatJet1Phi);
	  double jet2ToFatJet1_dR = sqrt(pow(jet2Eta-fatJet1Eta,2)+pow(jet2ToFatJet1_dphi,2));
	  double jet3ToFatJet1_dphi = (fabs(jet3Phi-fatJet1Phi) > TMath::Pi()) ? fabs(fabs(jet3Phi-fatJet1Phi) - 2*TMath::Pi()) : fabs(jet3Phi-fatJet1Phi);
	  double jet3ToFatJet1_dR = sqrt(pow(jet3Eta-fatJet1Eta,2)+pow(jet3ToFatJet1_dphi,2));
	  double jet4ToFatJet1_dphi = (fabs(jet4Phi-fatJet1Phi) > TMath::Pi()) ? fabs(fabs(jet4Phi-fatJet1Phi) - 2*TMath::Pi()) : fabs(jet4Phi-fatJet1Phi);
	  double jet4ToFatJet1_dR = sqrt(pow(jet4Eta-fatJet1Eta,2)+pow(jet4ToFatJet1_dphi,2));
	
	  if (jet1ToFatJet1_dR < 1.0) {	  
	    tmpDR_ClosestToFatJet1 = jet1ToFatJet1_dR;
	    jet1Eta_ClosestToFatJet1 = jet1Eta;
	    jet1Phi_ClosestToFatJet1 = jet1Phi;
	  } 
	  if (jet2ToFatJet1_dR < 1.0) {
	    if (jet2ToFatJet1_dR < tmpDR_ClosestToFatJet1) {
	      tmpDR_SecondClosestToFatJet1 = tmpDR_ClosestToFatJet1;
	      jet2Eta_ClosestToFatJet1 = jet1Eta_ClosestToFatJet1;
	      jet2Phi_ClosestToFatJet1 = jet1Phi_ClosestToFatJet1;

	      tmpDR_ClosestToFatJet1 = jet2ToFatJet1_dR;
	      jet1Eta_ClosestToFatJet1 = jet2Eta;
	      jet1Phi_ClosestToFatJet1 = jet2Phi;
	    } else {
	      tmpDR_SecondClosestToFatJet1 = jet2ToFatJet1_dR;
	      jet2Eta_ClosestToFatJet1 = jet2Eta;
	      jet2Phi_ClosestToFatJet1 = jet2Phi;
	    }
	  }
	  if (jet3ToFatJet1_dR < 1.0) {
	    if (jet3ToFatJet1_dR < tmpDR_ClosestToFatJet1) {
	      tmpDR_SecondClosestToFatJet1 = tmpDR_ClosestToFatJet1;
	      jet2Eta_ClosestToFatJet1 = jet1Eta_ClosestToFatJet1;
	      jet2Phi_ClosestToFatJet1 = jet1Phi_ClosestToFatJet1;

	      tmpDR_ClosestToFatJet1 = jet3ToFatJet1_dR;
	      jet1Eta_ClosestToFatJet1 = jet3Eta;
	      jet1Phi_ClosestToFatJet1 = jet3Phi;
	    } else if (jet3ToFatJet1_dR < tmpDR_SecondClosestToFatJet1) {
	      tmpDR_SecondClosestToFatJet1 = jet3ToFatJet1_dR;
	      jet2Eta_ClosestToFatJet1 = jet3Eta;
	      jet2Phi_ClosestToFatJet1 = jet3Phi;
	    }
	  }
	  if (jet4ToFatJet1_dR < 1.0) {
	    if (jet4ToFatJet1_dR < tmpDR_ClosestToFatJet1) {
	      tmpDR_SecondClosestToFatJet1 = tmpDR_ClosestToFatJet1;
	      jet2Eta_ClosestToFatJet1 = jet1Eta_ClosestToFatJet1;
	      jet2Phi_ClosestToFatJet1 = jet1Phi_ClosestToFatJet1;

	      tmpDR_ClosestToFatJet1 = jet4ToFatJet1_dR;
	      jet1Eta_ClosestToFatJet1 = jet4Eta;
	      jet1Phi_ClosestToFatJet1 = jet4Phi;
	    } else if (jet3ToFatJet1_dR < tmpDR_SecondClosestToFatJet1) {
	      tmpDR_SecondClosestToFatJet1 = jet4ToFatJet1_dR;
	      jet2Eta_ClosestToFatJet1 = jet4Eta;
	      jet2Phi_ClosestToFatJet1 = jet4Phi;
	    }
	  }
	  
	  cout << "Closest to Fat Jet 1: " << tmpDR_ClosestToFatJet1 << " " << tmpDR_SecondClosestToFatJet1  << "\n";

	  if (tmpDR_ClosestToFatJet1 < 1.0 && tmpDR_SecondClosestToFatJet1 < 1.0) {
	    double tmpDPhi = (fabs(jet1Phi_ClosestToFatJet1-jet2Phi_ClosestToFatJet1) > TMath::Pi()) ? fabs(fabs(jet1Phi_ClosestToFatJet1-jet2Phi_ClosestToFatJet1) - 2*TMath::Pi()) : fabs(jet1Phi_ClosestToFatJet1-jet2Phi_ClosestToFatJet1);
	    double tmpDR = sqrt(pow(jet1Eta_ClosestToFatJet1-jet2Eta_ClosestToFatJet1,2)+pow(tmpDPhi,2));
	    deltaR_ClosestToFatJet1 = tmpDR;
	    histFoundAK4DeltaR[i]->Fill(deltaR_ClosestToFatJet1);
	    cout << "DR = " << tmpDR << "\n";
	  }


	  //Find the AK4 jets closest to fatJet2
	  float deltaR_ClosestToFatJet2 = -999;
	  float tmpDR_ClosestToFatJet2 = 9999;
	  float tmpDR_SecondClosestToFatJet2 = 9999;
	  float jet1Eta_ClosestToFatJet2 = 0;
	  float jet1Phi_ClosestToFatJet2 = 0;
	  float jet2Eta_ClosestToFatJet2 = 0;
	  float jet2Phi_ClosestToFatJet2 = 0;

	  double jet1ToFatJet2_dphi = (fabs(jet1Phi-fatJet2Phi) > TMath::Pi()) ? fabs(fabs(jet1Phi-fatJet2Phi) - 2*TMath::Pi()) : fabs(jet1Phi-fatJet2Phi);
	  double jet1ToFatJet2_dR = sqrt(pow(jet1Eta-fatJet2Eta,2)+pow(jet1ToFatJet2_dphi,2));
	  double jet2ToFatJet2_dphi = (fabs(jet2Phi-fatJet2Phi) > TMath::Pi()) ? fabs(fabs(jet2Phi-fatJet2Phi) - 2*TMath::Pi()) : fabs(jet2Phi-fatJet2Phi);
	  double jet2ToFatJet2_dR = sqrt(pow(jet2Eta-fatJet2Eta,2)+pow(jet2ToFatJet2_dphi,2));
	  double jet3ToFatJet2_dphi = (fabs(jet3Phi-fatJet2Phi) > TMath::Pi()) ? fabs(fabs(jet3Phi-fatJet2Phi) - 2*TMath::Pi()) : fabs(jet3Phi-fatJet2Phi);
	  double jet3ToFatJet2_dR = sqrt(pow(jet3Eta-fatJet2Eta,2)+pow(jet3ToFatJet2_dphi,2));
	  double jet4ToFatJet2_dphi = (fabs(jet4Phi-fatJet2Phi) > TMath::Pi()) ? fabs(fabs(jet4Phi-fatJet2Phi) - 2*TMath::Pi()) : fabs(jet4Phi-fatJet2Phi);
	  double jet4ToFatJet2_dR = sqrt(pow(jet4Eta-fatJet2Eta,2)+pow(jet4ToFatJet2_dphi,2));
	
	  if (jet1ToFatJet2_dR < 1.0) {	  
	    tmpDR_ClosestToFatJet2 = jet1ToFatJet2_dR;
	    jet1Eta_ClosestToFatJet2 = jet1Eta;
	    jet1Phi_ClosestToFatJet2 = jet1Phi;
	  } 
	  if (jet2ToFatJet2_dR < 1.0) {
	    if (jet2ToFatJet2_dR < tmpDR_ClosestToFatJet2) {
	      tmpDR_SecondClosestToFatJet2 = tmpDR_ClosestToFatJet2;
	      jet2Eta_ClosestToFatJet2 = jet1Eta_ClosestToFatJet2;
	      jet2Phi_ClosestToFatJet2 = jet1Phi_ClosestToFatJet2;

	      tmpDR_ClosestToFatJet2 = jet2ToFatJet2_dR;
	      jet1Eta_ClosestToFatJet2 = jet2Eta;
	      jet1Phi_ClosestToFatJet2 = jet2Phi;
	    } else {
	      tmpDR_SecondClosestToFatJet2 = jet2ToFatJet2_dR;
	      jet2Eta_ClosestToFatJet2 = jet2Eta;
	      jet2Phi_ClosestToFatJet2 = jet2Phi;
	    }
	  }
	  if (jet3ToFatJet2_dR < 1.0) {
	    if (jet3ToFatJet2_dR < tmpDR_ClosestToFatJet2) {
	      tmpDR_SecondClosestToFatJet2 = tmpDR_ClosestToFatJet2;
	      jet2Eta_ClosestToFatJet2 = jet1Eta_ClosestToFatJet2;
	      jet2Phi_ClosestToFatJet2 = jet1Phi_ClosestToFatJet2;

	      tmpDR_ClosestToFatJet2 = jet3ToFatJet2_dR;
	      jet1Eta_ClosestToFatJet2 = jet3Eta;
	      jet1Phi_ClosestToFatJet2 = jet3Phi;
	    } else if (jet3ToFatJet2_dR < tmpDR_SecondClosestToFatJet2) {
	      tmpDR_SecondClosestToFatJet2 = jet3ToFatJet2_dR;
	      jet2Eta_ClosestToFatJet2 = jet3Eta;
	      jet2Phi_ClosestToFatJet2 = jet3Phi;
	    }
	  }
	  if (jet4ToFatJet2_dR < 1.0) {
	    if (jet4ToFatJet2_dR < tmpDR_ClosestToFatJet2) {
	      tmpDR_SecondClosestToFatJet2 = tmpDR_ClosestToFatJet2;
	      jet2Eta_ClosestToFatJet2 = jet1Eta_ClosestToFatJet2;
	      jet2Phi_ClosestToFatJet2 = jet1Phi_ClosestToFatJet2;

	      tmpDR_ClosestToFatJet2 = jet4ToFatJet2_dR;
	      jet1Eta_ClosestToFatJet2 = jet4Eta;
	      jet1Phi_ClosestToFatJet2 = jet4Phi;
	    } else if (jet3ToFatJet2_dR < tmpDR_SecondClosestToFatJet2) {
	      tmpDR_SecondClosestToFatJet2 = jet4ToFatJet2_dR;
	      jet2Eta_ClosestToFatJet2 = jet4Eta;
	      jet2Phi_ClosestToFatJet2 = jet4Phi;
	    }
	  }

	  cout << "Closest to Fat Jet 2: " << tmpDR_ClosestToFatJet2 << " " << tmpDR_SecondClosestToFatJet2  << "\n";

	  if (tmpDR_ClosestToFatJet2 < 1.0 && tmpDR_SecondClosestToFatJet2 < 1.0) {
	    double tmpDPhi = (fabs(jet1Phi_ClosestToFatJet2-jet2Phi_ClosestToFatJet2) > TMath::Pi()) ? fabs(fabs(jet1Phi_ClosestToFatJet2-jet2Phi_ClosestToFatJet2) - 2*TMath::Pi()) : fabs(jet1Phi_ClosestToFatJet2-jet2Phi_ClosestToFatJet2);
	    double tmpDR = sqrt(pow(jet1Eta_ClosestToFatJet2-jet2Eta_ClosestToFatJet2,2)+pow(tmpDPhi,2));
	    deltaR_ClosestToFatJet2 = tmpDR;
	    histFoundAK4DeltaR[i]->Fill(deltaR_ClosestToFatJet2);
	    cout << "DR = " << tmpDR << "\n";
	  }
  ...
  //MR
  //*******************************************************************************************
  PlotDataAndStackedBkg( histFoundAK4DeltaR, processLabels, color, true, "histFoundAK4DeltaR", totalLumi, Label);
  ...
  // // Output
  // //==============================================================================================================
  TFile *file = TFile::Open(("CheckOverlapWithResolved"+Label+".root").c_str(), "UPDATE");
  file->cd();

  for(int i=0; i<int(inputfiles.size()); i++) {
    file->WriteTObject(histFoundAK4DeltaR[i], Form("histFoundAK4DeltaR_%s",processLabels[i].c_str()), "WriteDelete");  
  } 
...
void CheckOverlapWithResolved( int option = -1) {
  ...
  std::string dir = "/eos/cms/store/group/phys_susy/razor/Run2Analysis/HH/HHTo4BNtupler/20200902/option0/combined/";
  ...
  bkgfiles_HH_2018.push_back(dir+"2018/GluGluToHHTo4B_node_cHHH1_TuneCP5_PSWeights_13TeV-powheg-pythia8.root");
