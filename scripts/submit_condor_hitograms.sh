...
if [ "$1" = "" ]; then
    ...
    echo "usage: source sumit_condor.sh <task_name> <input_list> <optional:--is_data>"
    echo "QUIT!!"
    exit 1
fi
...
#####################################
cp ${scripts_dir}/run_job_histograms.sh ${task_name_dir}/.
###################################################
...
printf "\n"
printf "\n\ngfal-copy -f *.root gsiftp://cmseos-gridftp.fnal.gov//"${eos_dir}/${task_name} >> ${task_name_dir}/run_job_histograms.sh
...
for i in `ls ${task_name_dir}`; do 
	...
	printf "Executable = "${task_name_dir}"/run_job_histograms.sh\n" >> ${condor_submit_file}
	...
	if [ "$_is_data" = true ] ; then
	    printf "Arguments = ${i} diHiggs_output_${task_name}_${ctr}.root --is_data \n" >> ${condor_submit_file}
	fi

	if [ "$_is_data" = false ] ; then
